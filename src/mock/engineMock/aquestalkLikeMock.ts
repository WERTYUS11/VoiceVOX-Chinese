...
enum ParseKanaErrorCode {
  UNKNOWN_TEXT = "有无法辨别的读假名: {text}",
  ACCENT_TOP = "句首不能放重音: {text}",
  ACCENT_TWICE = "一个重音短语不能有两个以上的重音: {text}",
  ACCENT_NOTFOUND = "有未指定重音的重音短语: {text}",
  EMPTY_PHRASE = "{position}第二个重音句为空",
  INTERROGATION_MARK_NOT_AT_END = "重音句末以外用“？”不能放置: {text}",
  INFINITE_LOOP = "处理时变成了无限循环…请报告错误。",
}
