...
function getWaveRate(phoneme: string): { [key in WaveType]: number } {
  ...
  // 無音ならほぼ無音
  if (phonemeFeatures.无音.includes(phoneme)) {
    const index = phonemeFeatures.无音.indexOf(phoneme);
  ...
  // 無声母音ならノイズ多め
  if (phonemeFeatures.无声母音.includes(phoneme)) {
    const rate =
      phonemeFeatures.无声母音.indexOf(phoneme) /
      (phonemeFeatures.无声母音.length - 1);
  ...
  // 無声子音ならノイズ多めで音量小さい
  if (phonemeFeatures.无声子音.includes(phoneme)) {
    const rate =
      phonemeFeatures.无声子音.indexOf(phoneme) /
      (phonemeFeatures.无声子音.length - 1);
    waveRate.sine = (1 - rate) * 0.1;
    waveRate.square = rate * 0.1;
    waveRate.noise = 0.1;
    return waveRate;
  ...
  throw new Error(`未对应的音素: ${phoneme}`);
}
