...
const contextMenuHeader = computed(() => {
  ...
  if (tempoChangeExists.value) {
    texts.push(`节奏：${currentTempo.value.bpm}`);
  }
  if (timeSignatureChangeExists.value) {
    texts.push(
      `节拍：${currentTimeSignature.value.beats}/${currentTimeSignature.value.beatType}`,
    );
  }
  return texts.join("，");
});
const contextMenuData = computed<ContextMenuItemData[]>(() => {
  ...
  menuData.push({
    type: "button",
    label: tempoChangeExists.value ? `节奏变化编辑` : "插入节奏变化",
    onClick: async () => {
      Dialog.create({
