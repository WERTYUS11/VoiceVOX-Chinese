...
process.on("uncaughtException", (error) => {
  ...
  if (isDevelopment) {
    ...
    let detailedMessage = "";
    detailedMessage += `主进程中发生不明原因错误。\n`;
    detailedMessage += `错误名称: ${name}\n`;
    detailedMessage += `信息: ${message}\n`;
    if (error.stack) {
  ...
  await configManager.initialize().catch(async (e) => {
      ...
      await dialog
        .showMessageBox({
          ...
          message: `加载配置文件失败。${app.getPath(
            "userData",
          )} 在 config.json 可以通过更改名称来解决（但所有设置都会重置）。打开包含配置文件的文件夹？`,
          buttons: ["不！", "好！"],
          noLink: true,
          cancelId: 0,
        })
    ...
    // 開発時はconfigをリセットして起動を続行するかも問う
      ...
      await dialog
        .showMessageBox({
          type: "error",
          title: "设置文件读取错误（面向开发者的指南）",
          message: `读取配置文件失败。请更改配置文件的名称或重置设置。`,
          buttons: [
            "关闭",
            "打开设置文件夹",
            "重置设置并继续",
